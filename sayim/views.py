...
# --- YENİ EKLENEN/GÜNCELLENEN IMPORT'LAR (GEMINI İÇİN) ---
...
def raporlama_onay(request, sayim_emri_id):
    """Raporlama ve Onay Ekranı. (FieldError Çözüldü)"""
    # Session kontrolünü manuel yapıyoruz
    if 'current_user' not in request.session:
        return redirect('personel_login')
    ...
    # *** REVİZYON YAPILAN KISIM: 'malzeme__' yerine 'benzersiz_malzeme__' kullanıldı. (FieldError çözüldü) ***
    sayim_ozet = SayimDetay.objects.filter(sayim_emri=sayim_emri).values(
        'benzersiz_malzeme__malzeme_kodu', 'benzersiz_malzeme__malzeme_adi', 'benzersiz_malzeme__parti_no',
        'benzersiz_malzeme__renk', 'benzersiz_malzeme__sistem_stogu', 'benzersiz_malzeme__olcu_birimi'
    ).annotate(
    ...
    for ozet in sayim_ozet:
        # Key'ler de 'benzersiz_malzeme__' olarak güncellendi.
        sistem_stok = ozet['benzersiz_malzeme__sistem_stogu'] or Decimal('0.00')
        sayim_miktar = ozet['toplam_sayim_miktar'] or Decimal('0.00')
        fark = sayim_miktar - sistem_stok
        
        rapor_verileri.append({
            'stok_kod': ozet['benzersiz_malzeme__malzeme_kodu'],
            'malzeme_adi': ozet['benzersiz_malzeme__malzeme_adi'],
            'parti_no': ozet['benzersiz_malzeme__parti_no'] if ozet['benzersiz_malzeme__parti_no'] != 'YOK' else '-',
            'renk': ozet['benzersiz_malzeme__renk'] if ozet['benzersiz_malzeme__renk'] != 'YOK' else '-',
            'olcu_birimi': ozet['benzersiz_malzeme__olcu_birimi'],
...
def ajax_akilli_stok_ara(request):
    """
    Barkod, Stok Kodu, Parti No ve Renk'e göre Malzeme arar. (Seri No araması eklendi)
    """
    ...
    malzeme = None
    if seri_no != 'YOK':
        # *** REVİZYON: Seri Numarası artık Malzeme modelinin 'seri_no' alanında da aranıyor. ***
    ...
    if malzeme:
        ...
        # Sayım detayında 'malzeme' yerine 'benzersiz_malzeme' kullanıldığını varsayıyoruz.
        sayilan_miktar = SayimDetay.objects.filter(sayim_emri=sayim_emri, benzersiz_malzeme=malzeme).aggregate(Sum('miktar'))['miktar__sum'] or Decimal('0.00')
...
def ajax_sayim_kaydet(request, sayim_emri_id):
    ...
    try:
        ...
        SayimDetay.objects.create(
            sayim_emri=sayim_emri, benzersiz_malzeme=malzeme, miktar=miktar, # *** REVİZYON: 'malzeme' yerine 'benzersiz_malzeme' kullanıldı. ***
            personel_adi=personel_adi, kayit_tarihi=timezone.now(),
            lokasyon_lat=lat, lokasyon_lon=lon
        )
        
        toplam_sayilan_miktar = SayimDetay.objects.filter(sayim_emri=sayim_emri, benzersiz_malzeme=malzeme).aggregate(Sum('miktar'))['miktar__sum'] or Decimal('0.00')
...
def gemini_ocr_analiz(request):
    """
    Yüklenen görseldeki barkodları, seri noları ve miktarları Gemini Vision ile okur.
    (Tam Entegrasyon)
    """
    ...
    try:
        ...
        model = genai.GenerativeModel('gemini-2.5-flash') # Hızlı ve Multimodal
        
        # 2. Görseli Belleğe Oku ve Multimodal Parça Oluştur
        ...
        # 3. Prompt (İstek) Oluştur
        # Modelden temiz bir JSON çıktısı almak için özel talimatlar
        prompt = (
            "Bu, bir stok sayım etiketidir. Lütfen sadece JSON formatında yanıt ver. "
            "JSON nesnesinde sadece bulduğun 'Stok Kodu', 'Seri No', 'Parti No' ve 'Miktar' değerlerini kullan. "
            "Değer bulamazsan 'YOK' yaz. Yanıtın başında veya sonunda metin olmasın, sadece JSON çıktısı olsun. "
            "Örnek format: {\"stok_kod\": \"1234\", \"seri_no\": \"SN-5678\", \"parti_no\": \"LOT-90\", \"miktar\": \"15.00\"}"
        )
        
        # 4. Gemini'a Gönder
        response = model.generate_content([prompt, image_part])

        # 5. Yanıtı Temizle ve JSON olarak parse et
        # Bazen Gemini, çıktıyı ```json ... ``` içine koyabiliyor. Onları temizleyelim.
        ...
        return JsonResponse({
            'success': True, 
            'message': 'OCR analizi başarıyla tamamlandı.',
            'results': ocr_result
        })

    except json.JSONDecodeError:
        return JsonResponse({'success': False, 'message': f'HATA: Gemini tarafından gelen metin JSON formatında değildi.'}, status=500)
    except Exception as e:
        print(f"Gemini OCR Kritik Hata: {e}")
...
def export_mutabakat_excel(request, sayim_emri_id):
    ...
    try:
        ...
        for detay in SayimDetay.objects.filter(sayim_emri=sayim_emri):
            # *** REVİZYON: 'detay.malzeme' yerine 'detay.benzersiz_malzeme' kullanıldı. ***
